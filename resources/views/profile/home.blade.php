...
<div class="profile-info">
    <div class="profile-info__left">
        <a >
            <img class='profile-info__image' src="{{ $user->profile_photo_path?asset($user->profile_photo_path):$user->profile_photo_url}}" alt="">
        </a>
    </div>
    ...
    <div class="profile-info__right-up">
        <div class="user-options">
            ...
            <a href="{{ url('/accounts/edit')}}" class='edit-btn'>Edit Profile</a>
            <a href="{{ url('/accounts/edit')}}" class='my-link'>
            <svg aria-label="Options" class="_8-yf5 " fill="#349713" height="24" viewBox="0 0 48 48" width="24">
            ...
            <a href="{{ url('/accounts/edit')}}" class='edit-btn m-width'>Edit Profile</a>
            @else
            <span class='icon-options'>
                ...
                <a class='unblock-user friend-action edit-btn' href="{{url('/user/unblock/'.$user->id)}}" class='edit-btn'>Unblock</a>
                @endif
                @if( !$relationship || $relationship->status !== 3)
                <a href="{{url('/user/block/'.$user->id)}}" class='block-user-icon'><i class=" clickable fas  fa-ban"></i></a>
                @else
                <a href="{{url('/user/block/'.$user->id)}}" class='hide block-user-icon'><i class=" clickable fas  fa-ban"></i></a>
                @endif
            </span>
            @endif
        </div>
    ...
    <ul>
        <li class='<?= $page === 'home' ? 'active' : '' ?>'>
            ...
            <a href="{{url('/user/'.$user->username)}}">
                <span>
                    <svg aria-label="Posts" class="_8-yf5 " fill="#349713" height="12" viewBox="0 0 48 48" width="12">
...
@if(($user->id===Auth::user()->id) || ($user->privacy === 0) || (isset($relationship->status) && $relationship->status === 1 ))
<!-- POSTS -->
